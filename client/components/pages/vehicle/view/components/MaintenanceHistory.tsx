    ...
    <Card className="border-border shadow-sm">
      ...
      <CardContent>
        <div className="space-y-4">
          {history.map((record) => (
            <div key={record.id} className="flex items-start justify-between p-3 bg-muted/50 rounded-lg">
              <div>
                ...
                <p className="text-sm font-medium text-foreground mb-1">{record.description}</p>
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-muted-foreground">By: {record.completedBy}</span>
                  <span className="text-muted-foreground">Cost: ₦{record.cost.toLocaleString()}</span>
                  {record.nextDue && (
