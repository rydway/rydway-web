...
export default function AdminOverviewPage() {
  ...
  const metrics = summary?.data ? {
    totalUsers: summary.data.users?.total ?? 0,
    totalVehicles: summary.data.vehicles?.total ?? 0,
    activeBookings: summary.data.bookings?.active ?? 0,
    totalRevenue: summary.data.revenue?.total ?? 0,
  } : {
    totalUsers: 1420,
    totalVehicles: 345,
    activeBookings: 89,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground dark:text-white">Platform Overview</h1>
        <p className="text-muted-foreground font-secondary mt-1">High-level metrics across the Rydway ecosystem.</p>
      </div>
