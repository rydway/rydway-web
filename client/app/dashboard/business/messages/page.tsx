...
export default function BusinessMessagesPage() {
  ...
  const currentUser: User = {
    ...
    name: user?.firstName ? `${user.firstName} ${user.lastName}` : "Business",
  ...
  const mappedConversations: Conversation[] = conversations.map((c: any) => ({
    id: c.id,
    otherParty: {
      id: c.renter?.id || "unknown",
      name: c.renter?.firstName ? `${c.renter.firstName} ${c.renter.lastName}` : "Renter",
      avatar: c.renter?.profileImageUrl || "/api/placeholder/32/32",
      role: "renter"
    },
    booking: {
