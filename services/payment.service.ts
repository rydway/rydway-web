...
export const paymentService = {
  ...
  async initializePayment(data: {
    ...
    const response = await api.post<{ data: PaymentInitResponse }>('/payments/initialize', {
  ...
  async verifyPayment(transactionRef: string): Promise<Payment> {
    const response = await api.post<{ data: Payment }>('/payments/verify', { transactionRef });
    return response.data;
  },
  ...
  async getTransactions(): Promise<Payment[]> {
    const response = await api.get<{ data: Payment[] }>('/payments/transactions');
    return response.data;
  },
