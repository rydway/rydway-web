...
let BookingsService = class BookingsService {
    ...
    async markActive(hostUserId, bookingId) {
        ...
        if (!preInspection) {
            throw new common_1.BadRequestException('Pre-trip inspection photos must be uploaded before starting the trip');
        }
        const updated = await this.prisma.booking.update({
            where: { id: bookingId },
            data: {
    ...
    async completeBooking(hostUserId, bookingId) {
        ...
        if (!postInspection) {
            throw new common_1.BadRequestException('Post-trip inspection photos must be uploaded before completing the trip');
        }
        const updated = await this.prisma.booking.update({
            where: { id: bookingId },
            data: {
